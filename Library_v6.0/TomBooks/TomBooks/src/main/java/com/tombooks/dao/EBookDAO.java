...
public class EBookDAO {
    ...
    public int countEbooks() {
        Query query = em.createQuery("SELECT COUNT(u) FROM Ebook e");
    ...
    public int countEBooks() {
        Query query = em.createQuery("SELECT COUNT(e) FROM Ebook e");
        return ((Long) query.getSingleResult()).intValue();
    }

    public List<Ebook> findEBooks(int offset, int pageSize) {
        Query query = em.createQuery("SELECT e FROM Ebook e");
